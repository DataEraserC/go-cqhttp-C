...
static int AnalysisJSON(JSON **node, int index, char *JSONString) {
  ...
  preLength = index;
  index++; //自增操作是因为此时索引的字符是 " 或者
           //{，直接带入下一行的函数会返回0
  if (JSONString[index] == '\"') //如果此时为 " 则此 " 为字符串开头 " 直接跳过
    index++;

  length = getLengthOfNextChar(
      JSONString, index,
      '\"'); //获取到对象名结束的双引号之前的字符的长度（不包括双引号）
  name = interceptString(JSONString, index, length);

  index =
      index + length + 2; //当前索引 + 对象名长度 + 对象名后 ": 两个字符的长度
  switch (
      JSONString[index]) //检索冒号后的第一个字符（对象的值），以判断数据类型
  {
  case '\"': //字符串类型
...
static int AnalysisJSONString(JSON **node, int index, char *JSONString) {
  char *value;
  int length = getLengthOfNextChar(
      JSONString, index + 1,
      '\"'); //+1操作是因为此时索引的字符是"，直接带入此函数会返回0
...
//	//recursionData->index++;
//	//switch
//(recursionData->string[recursionData->index])//检索冒号后的第一个字符（对象的值），以判断数据类型
//	//{//TODO 把这几个解析函数重新写一遍适配数组的
//	//case '\"'://字符串类型
//	//	ArrayNode->head =
//(JSONArrayNode*)malloc_s(sizeof(JSONArrayNode));
//
//	//	AnalysisJSONStringArray(node, recursionData->string,
//&CurrentIndex);
//	//	break;
//	//case '1'://整数类型
//	//case '2':
...
//	//case '9':
//	//case '0':
//	//	AnalysisJSONIntArray(node, recursionData->string,
//&CurrentIndex);
//	//	break;
//	//case '['://数组类型
//	//	//AnalysisJSONArray(node, recursionData->string, &CurrentIndex);
...
static char *interceptString(const char *str, int startIndex, int size) {
  ...
  for (index = 0, offset = 0; index < size; index++, offset++) {
    ...
    {
      returnString[index] =
          str[startIndex + ++offset]; //直接跳过当前转义符，并用下一个字符赋值
    } else {
...
char *GetStringDuplicate(const char *string) {
  ...
  strncpy(duplicate, string, len + 1);
  duplicate[len + 1] = '\0';
...
//释放JSON数据
void DestoryJSON(JSON *data) { FreeJSON(data); }
  ...
  while (con) {
    if (!strcmp(name, con->name)) {
      if (con->type != StringType)
        return -1;
      strncpy(value, con->data.stringData, strlen(con->data.stringData) + 1);
      value[strlen(con->data.stringData) + 1] = '\0';
      return 0;
    }
    con = con->next;
  }
