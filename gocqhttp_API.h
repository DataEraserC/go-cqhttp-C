...
// API
void send_private_msg(send_private_msg_data *data //发包
);

//获取发包
send_private_msg_data New_send_private_msg(unsigned int user_id,  //用户ID
...
#define API_SEND_GROUP_MSG_FORM                                                \
  "GET /send_group_msg?group_id=%u&message=%s&auto_escape=%d "                 \
  "HTTP/1.1\r\nHost: 127.0.0.1:5700\r\nConnection: keep-alive\r\n\r\n"
...
// API
void send_group_msg(send_group_msg_data *data //发包
);

//获取发包
send_group_msg_data New_send_group_msg(unsigned int group_id, //群号
                                       char message[1024],    //消息
                                       int auto_escape        //是否纯文本
);
...
#define API_GET_MSG_RECV                                                       \
  "%*[^{]{\"data\":{\"group\":%[^,],\"group_id\":%u,\"message\":\"%[^\"]\","   \
  "\"message_id\":%d,\"message_id_v2\":\"%[^\"]\",\"message_seq\":%d,"         \
  "\"message_type\":\"%[^\"]\",\"real_id\":%d,\"sender\":{\"nickname\":\"%[^"  \
...
// API
void get_msg(get_msg_data *data //发包
);
...
// API
void delete_msg(delete_msg_data *data //发包
);
...
#define API_GET_GROUP_MEMBER_INFO_FORM                                         \
  "GET /get_group_member_info?group_id=%lu&user_id=%lu&no_cache=%d "           \
  "HTTP/1.1\r\nHost: 127.0.0.1:5700\r\nConnection: keep-alive\r\n\r\n"
#define API_GET_GROUP_MEMBER_INFO_RECV                                         \
  "%*[^{]{\"data\":{\"age\":%d,\"area\":%[^,],\"card\":\"%[^\"]\",\"card_"     \
  "changeable\":%[^,],\"group_id\":%lu,\"join_time\":%d,\"last_sent_time\":%"  \
  "d,\"level\":\"%[^\"]\",\"nickname\":\"%[^\"]\",\"role\":\"%[^\"]\","        \
  "\"sex\":\"%[^\"]\",\"shut_up_timestamp\":%d,\"title\":%[^,],\"title_"       \
  "expire_time\":%lu,\"unfriendly\":%[^,],\"user_id\":%lu},\"retcode\":%d,"    \
  "\"status\":\"%[^\"]\"}"
