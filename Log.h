...
void logInfoDetailed(char *functionName, char *filePath, int line,
                     char *message, ...);
#define logInfoAll(message, ...)                                               \
  logInfoDetailed(__func__, __FILE__, __LINE__, message, __VA_ARGS__)

//警告输出
void logWarn(char *message, ...);
void logWarnDetailed(char *functionName, char *filePath, int line,
                     char *message, ...);
#define logWarnAll(message, ...)                                               \
  logWarnDetailed(__func__, __FILE__, __LINE__, message, __VA_ARGS__)

//错误输出
void logErr(char *message, ...);
void logErrDetailed(char *functionName, char *filePath, int line, char *message,
                    ...);
#define logErrAll(message, ...)                                                \
  logErrDetailed(__func__, __FILE__, __LINE__, message, __VA_ARGS__)
